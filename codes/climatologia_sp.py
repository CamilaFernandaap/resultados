...
def plot_climatologia_mapas(clim_dict, titulo, unidade, cmap, shp, out):
    ...
    for ax, e in zip(axes.flat, ["DJF", "MAM", "JJA", "SON"]):
        ...
        ax.set_title(f"{e}", fontsize=18, fontweight='bold')
    ...
    fig.colorbar(im, cax=cax).set_label(unidade, fontsize=14)
    
    fig.suptitle(f"Normal Climatológica (1990-2024) - {titulo} - São Paulo", 
                 fontsize=22, fontweight='bold', y=0.96)
    fig.subplots_adjust(left=0.05, right=0.90, bottom=0.08, top=0.92, hspace=0.25, wspace=0.25)
    
    plt.savefig(f"{out}/Climatologia_{titulo}_SP_mapas.png", dpi=300, bbox_inches="tight")
    plt.show()
    print(f"  Salvo: Climatologia_{titulo}_SP_mapas.png")
...
def plot_climatologia_estatisticas(clim_dict, titulo, unidade, shp, out):
    ...
    ax1.set_title(f"Média Climatológica", fontsize=16, fontweight='bold')
    ax1.set_xlim(-53.5, -44.0)
    ax1.set_ylim(-25.5, -19.5)
    ...
    ax2.set_title(f"Desvio Padrão", fontsize=16, fontweight='bold')
    ax2.set_xlim(-53.5, -44.0)
    ax2.set_ylim(-25.5, -19.5)
    ...
    ax3.set_title(f"Mínimo (1990-2024)", fontsize=16, fontweight='bold')
    ax2.set_xlim(-53.5, -44.0)
    ax2.set_ylim(-25.5, -19.5)
    ...
    ax4.set_title(f"Máximo (1990-2024)", fontsize=16, fontweight='bold')
    ax4.set_xlim(-53.5, -44.0)
    ax4.set_ylim(-25.5, -19.5)
    
    fig.suptitle(f"Estatísticas Climatológicas - {titulo} ({estacao}) - São Paulo", 
                 fontsize=22, fontweight='bold', y=0.96)
    ...
    plt.show()
    print(f"  Salvo: Climatologia_{titulo}_SP_estatisticas_{estacao}.png")
...
def exportar_estatisticas_csv(clim_temp, clim_rad, clim_prec, out):
    ...
    print(f"  Salvo: Climatologia_SP_estatisticas.csv")
...
if __name__ == "__main__":
    print("="*80)
    print(" "*15 + "NORMAL CLIMATOLÓGICA - São Paulo")
