...
def mascara_sp(da, shp):
    """Cria máscara para o estado de São Paulo"""
    gdf = gpd.read_file(shp)
    sp = gdf[gdf["sigla"] == "SP"].geometry.union_all()
    mask = np.zeros(da.shape, bool)
    for i, lat in enumerate(da.lat.values):
        for j, lon in enumerate(da.lon.values):
            mask[i, j] = sp.contains(Point(lon, lat))
    return mask
...
def selecionar_pontos_geograficos_sp(res_temp, shp):
    """
    Seleciona 10 pontos estrategicamente distribuídos pelo estado de São Paulo
    """
    pontos_alvo = [
        (-19.8, -50.4, "Noroeste"),      # Extremo noroeste
        (-20.5, -47.4, "Norte"),          # Norte
        (-20.2, -44.9, "Nordeste"),       # Extremo nordeste
        (-22.3, -52.6, "Oeste"),          # Extremo oeste
        (-22.9, -48.5, "Centro"),         # Centro do estado
        (-22.5, -45.0, "Leste"),          # Leste
        (-24.7, -53.0, "Sudoeste"),       # Extremo sudoeste
        (-24.0, -48.0, "Sul"),            # Sul
        (-24.3, -46.9, "Sudeste"),        # Sudeste
        (-23.5, -46.6, "Capital"),        # São Paulo capital
    ]
    
    slope = res_temp["DJF"]["slope"]
    mask_sp = mascara_sp(slope, shp)
    ...
    for lat_alvo, lon_alvo, regiao in pontos_alvo:
        ...
        # Cria máscara de pontos válidos em SP
        valid_mask = mask_sp & ~np.isnan(slope.values)
...
if __name__ == "__main__":
    print("="*80)
    print(" " * 20 + "ANÁLISE INTEGRADA - SÃO PAULO")
    ...
    out = r"F:\resultados_SP_10pontos"
    ...
    print("\n" + "="*80)
    print("SELECIONANDO 10 PONTOS DISTRIBUÍDOS POR SÃO PAULO")
    print("="*80)
    
    pontos = selecionar_pontos_geograficos_sp(res_t, shp)
